<x-app-layout title="Edit Profil Admin – Jejak Hijau" :user="$user">
    <div class="px-4 py-6 max-w-xl mx-auto space-y-6 animate-fade-in">
        ...
        <div class="flex items-center gap-2 border-b border-gray-150 pb-5">
            <a href="{{ route('admin.profile.show') }}" class="p-2 rounded-xl bg-gray-50 border border-gray-200 hover:bg-gray-100 transition-colors text-gray-500 hover:text-gray-700">
        ...
        {{-- Form --}}
        <form id="edit-profile-form" method="POST" action="{{ route('admin.profile.update') }}" enctype="multipart/form-data" class="bg-white rounded-2xl p-6 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-6" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" placeholder="Contoh: Super Admin" required
            ...
            <div class="flex items-center justify-end gap-4 border-t border-gray-100 pt-5">
                <a href="{{ route('admin.profile.show') }}" class="px-5 py-2.5 rounded-xl border border-gray-250 text-sm font-semibold text-gray-700 bg-white hover:bg-gray-50 transition-all">
                    Batal
                </a>
