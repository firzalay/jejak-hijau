<x-app-layout title="Profil Admin – Jejak Hijau" :user="$user">
    <div class="space-y-6 max-w-2xl mx-auto pb-10">
        {{-- Header Section --}}
        <section class="animate-fade-in-up flex items-center justify-between">
            <div>
                <h2 class="font-bold text-2xl" style="color: #111827;">Profil Admin</h2>
                <p class="text-sm mt-1" style="color: #6B7280;">
                    Kelola informasi profil, pantau data tinjauan organizer, dan amankan akun Anda.
                </p>
            </div>
            <div>
        ...
        <section class="grid grid-cols-3 gap-4 animate-fade-in-up">
            ...
            <div class="bg-white p-4 rounded-2xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-1">
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Organizer Disetujui</span>
                <p class="text-xl font-black text-emerald" id="stat-approved-organizers" style="color: #2ECF89;">{{ $approvedCount }}</p>
            </div>
            {{-- Rejected Organizers --}}
            <div class="bg-white p-4 rounded-2xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-1">
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Organizer Ditolak</span>
                <p class="text-xl font-black text-red-500">{{ $rejectedCount }}</p>
            </div>
            {{-- Total Reviews --}}
            <div class="bg-white p-4 rounded-2xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-1">
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Total Tinjauan</span>
                <p class="text-xl font-black text-gray-800" id="stat-total-reviews">{{ $totalReviews }}</p>
            </div>
        </section>
        ...
        <section class="bg-white rounded-2xl p-5 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-4 animate-fade-in-up">
            ...
            <form method="POST" action="{{ route('admin.profile.update-password') }}" class="space-y-4" novalidate>
                @csrf
                @method('PUT')
