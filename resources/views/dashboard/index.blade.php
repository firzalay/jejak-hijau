<x-app-layout title="Dashboard – GreenMile" :user="$user">
        ...
        <div class="lg:grid lg:grid-cols-3 lg:gap-6 space-y-5 lg:space-y-0">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <section class="animate-fade-in-up animate-delay-100">
                    ...
                    <x-points-summary :current-event-points="$activeParticipation->current_event_points" :total-points="$totalPoints" />
                </section>
                ...
                <section class="animate-fade-in-up animate-delay-150">
                    <h2 class="font-bold text-sm mb-3 flex items-center gap-2" style="color: #374151;">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <line x1="18" y1="20" x2="18" y2="10" stroke="#6B7280"
                                stroke-width="2" stroke-linecap="round" />
                            <line x1="12" y1="20" x2="12" y2="4" stroke="#6B7280"
                                stroke-width="2" stroke-linecap="round" />
                            <line x1="6" y1="20" x2="6" y2="14" stroke="#6B7280"
                                stroke-width="2" stroke-linecap="round" />
                        </svg>
                        Statistik Event
                    </h2>
                    <div class="grid grid-cols-3 gap-3">
                        {{-- Peringkat --}}
                        <div
                            class="rounded-2xl p-4 bg-white shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
                            <div class="w-8 h-8 rounded-xl flex items-center justify-center mb-3"
                                style="background: rgba(245,158,11,0.12); color: #F59E0B;">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path d="M6 9H3V3h3M18 9h3V3h-3M8 21h8M12 17v4" stroke="currentColor"
                                        stroke-width="2" stroke-linecap="round" />
                            ...
                            <p class="font-bold text-lg leading-none" style="color: #111827;">#{{ $userRank ?: '-' }}
                            </p>
                            <p class="text-xs mt-1 text-amber-500">dari
                                {{ $activeParticipation->event->participants()->count() }}</p>
                        </div>
                        ...
                        <div
                            class="rounded-2xl p-4 bg-white shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
                            <div class="w-8 h-8 rounded-xl flex items-center justify-center mb-3"
                                style="background: rgba(46,207,137,0.12); color: #2ECF89;">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                            ...
                            <p class="font-bold text-lg leading-none" style="color: #111827;">
                                {{ $activeParticipation->completed_checkpoints }}</p>
                ...
                @if ($leaderboardPreview !== null && $leaderboardPreview->isNotEmpty())
                    <section class="animate-fade-in-up animate-delay-300">
                        <h2 class="font-bold text-sm mb-3 flex items-center gap-2" style="color: #374151;">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path d="M6 9H3V3h3M18 9h3V3h-3M8 21h8M12 17v4" stroke="#6B7280" stroke-width="2"
                                    stroke-linecap="round" />
                                <path d="M6 3h12v8a6 6 0 0 1-12 0V3z" stroke="#6B7280" stroke-width="2" />
                            </svg>
                            Top Peserta
                        </h2>
                        <x-mini-leaderboard :participants="$leaderboardPreview" :user-rank="$userRank" :current-user-id="$user->id" />
                    </section>
                @endif

                {{-- Exit Event Action --}}
                <div class="mt-5 animate-fade-in-up animate-delay-350">
                    <form action="{{ route('events.exit', $activeParticipation->event_id) }}" method="POST"
                        onsubmit="return confirm('Apakah Anda yakin ingin keluar dari event ini? Semua poin dan progres Anda pada event ini akan dihapus secara permanen.')"
                        style="margin: 0; width: 100%;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" id="btn-dashboard-exit-event"
        ...
        <section class="flex flex-col items-center justify-center py-16 text-center animate-fade-in-up">
            ...
            <div class="w-28 h-28 rounded-full flex items-center justify-center mb-6"
                ...
                <svg width="52" height="52" viewBox="0 0 24 24" fill="none"
                    ...
                    <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke="#003F2F" stroke-width="1.5"
                        stroke-linejoin="round" />
                </svg>
            </div>
