<x-app-layout title="Tambah Checkpoint Baru – Jejak Hijau" :user="auth()->user()">
    <div class="px-4 py-6 max-w-3xl mx-auto space-y-6">
        ...
        <form id="create-checkpoint-form" method="POST" action="{{ route('organizer.events.checkpoints.store', $event->id) }}" class="bg-white rounded-2xl p-6 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-6" novalidate>
            ...
            <div class="space-y-1.5">
                ...
                @enderror
            </div>            {{-- Checkpoint Points Configuration Section --}}
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center gap-2 cursor-pointer select-none text-sm font-semibold text-gray-700">
                            <input type="checkbox" id="is_custom_point" name="is_custom_point" value="1" {{ old('is_custom_point') ? 'checked' : '' }}
                                   class="rounded border-gray-300 text-emerald focus:ring-emerald/20">
                            <span>Kustomisasi Poin Manual</span>
                        </label>
                        
                        <div class="space-y-1.5">
                            <input type="number" id="points" name="points" value="{{ old('points') }}" min="1"
                ...
                <div class="space-y-4 pt-4 border-t border-gray-150">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-gray-400">Atur berapa persen dari pool checkpoint yang diterima peserta berdasarkan urutan/rank scan.</p>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const totalPointPool = {{ (int) $event->total_point_pool }};
            const existingCheckpoints = {!! json_encode($event->checkpoints->map(fn($cp) => ['id' => $cp->id, 'point' => $cp->point, 'is_custom' => (bool)$cp->is_custom_point])) !!};
            ...
            function updatePointsField() {
                ...
                if (isCustom) {
                    ...
                    if (pointsInput.value === '' || pointsInput.value == alloc.autoPoint) {
                        pointsInput.value = alloc.autoPoint > 0 ? alloc.autoPoint : 1;
                    }
