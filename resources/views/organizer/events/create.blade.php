<x-app-layout title="Buat Event Baru – Jejak Hijau" :user="$user">
    <div class="px-4 py-6 max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-2">
            ...
            <div>
                <h1 class="text-2xl font-black text-gray-900 tracking-tight">Buat Event Baru</h1>
                <p class="text-sm text-gray-500 mt-1">Draf event baru untuk aksi GreenRun Anda.</p>
            </div>
        </div>

        {{-- Form --}}
        <form id="create-event-form" method="POST" action="{{ route('organizer.events.store') }}" class="bg-white rounded-2xl p-6 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-6" novalidate>
            @csrf

            {{-- Event Name --}}
            <div class="space-y-1.5">
                <label for="name" class="block text-sm font-semibold text-gray-700">Nama Event <span class="text-red-500">*</span></label>
                <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: GreenRun Surabaya Eco-Sprint" required
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" id="location" name="location" value="{{ old('location') }}" placeholder="Contoh: Taman Bungkul, Surabaya" required
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" id="banner" name="banner" value="{{ old('banner') }}" placeholder="Contoh: https://images.unsplash.com/photo-..."
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" id="total_rewards" name="total_rewards" value="{{ old('total_rewards') }}" placeholder="Contoh: Rp 15.000.000"
                ...
                {{-- Max Points --}}
                <div class="space-y-1.5">
                    <label for="max_points" class="block text-sm font-semibold text-gray-700">Maks. Poin <span class="text-red-500">*</span></label>
                    <input type="number" id="max_points" name="max_points" value="{{ old('max_points', 500) }}" min="1" required
                           class="w-full px-4 py-2.5 rounded-xl border {{ $errors->has('max_points') ? 'border-red-500 focus:ring-red-200' : 'border-gray-300 focus:border-emerald focus:ring-emerald/20' }} focus:outline-none focus:ring-4 transition-all text-sm text-gray-800">
                    @error('max_points')
                        <p class="text-xs text-red-500 mt-1 flex items-center gap-1">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                            {{ $message }}
            ...
            <div class="flex items-center justify-end gap-4 border-t border-gray-100 pt-5">
                <a href="{{ route('organizer.events.index') }}" class="px-5 py-2.5 rounded-xl border border-gray-250 text-sm font-semibold text-gray-700 bg-white hover:bg-gray-50 transition-all">
                    Batal
                </a>
