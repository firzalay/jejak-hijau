<x-app-layout title="Profil Organisasi – GreenMile" :user="$user">
    <div class="space-y-6 max-w-2xl mx-auto pb-10">
        ...
        <section class="animate-fade-in-up flex items-center justify-between">
            <div>
                <h2 class="font-bold text-2xl" style="color: #111827;">Profil Organisasi</h2>
                <p class="text-sm mt-1" style="color: #6B7280;">
                    Kelola detail organisasi, pantau data event, dan perbarui informasi profil Anda.
                </p>
            </div>
            <div>
        ...
        {{-- Statistics Grid --}}
        <section class="grid grid-cols-3 gap-4 animate-fade-in-up">
            {{-- Total Events --}}
            <div class="bg-white p-4 rounded-2xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-1">
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Total Event</span>
                <p class="text-xl font-black text-gray-800" id="stat-total-events">{{ $totalEvents }}</p>
            </div>
            {{-- Active Events --}}
            <div class="bg-white p-4 rounded-2xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-1">
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Event Aktif</span>
                <p class="text-xl font-black text-emerald" id="stat-active-events" style="color: #2ECF89;">{{ $activeEvents }}</p>
            </div>
            {{-- Total Participants --}}
            <div class="bg-white p-4 rounded-2xl shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-1">
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-wider block">Total Peserta</span>
                <p class="text-xl font-black text-gray-800" id="stat-total-participants">{{ $totalParticipants }}</p>
            </div>
        </section>
        ...
        <section class="bg-white rounded-2xl p-5 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-4 animate-fade-in-up">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                    <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider">Nama Organisasi</span>
                    <p class="font-semibold text-gray-800 mt-0.5" id="org-name">{{ $user->organizerProfile->organization_name ?? '-' }}</p>
                </div>
                <div>
                    <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider">Kontak Person</span>
                    <p class="font-semibold text-gray-800 mt-0.5" id="org-contact">{{ $user->organizerProfile->contact_person ?? '-' }}</p>
                </div>
                <div>
                    <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider">Nomor Telepon</span>
                    <p class="font-semibold text-gray-800 mt-0.5" id="org-phone">{{ $user->organizerProfile->phone ?? '-' }}</p>
                </div>
                <div>
                    <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider">Website</span>
                    @if($user->organizerProfile && $user->organizerProfile->website)
                        <a href="{{ $user->organizerProfile->website }}" target="_blank" rel="noopener noreferrer" class="block font-semibold text-emerald hover:underline mt-0.5" style="color: #2ECF89;" id="org-website">
                            {{ $user->organizerProfile->website }}
                        </a>
                    @else
                        <p class="font-semibold text-gray-800 mt-0.5" id="org-website">-</p>
                    @endif
                </div>
            </div>

            <div class="pt-2 text-sm">
                <span class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Deskripsi Organisasi</span>
                <p class="text-gray-650 leading-relaxed" id="org-description">
                    {{ $user->organizerProfile->description ?? 'Tidak ada deskripsi organisasi.' }}
                </p>
            </div>
        </section>
        ...
        <section class="bg-white rounded-2xl p-5 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-4 animate-fade-in-up">
            ...
            <form method="POST" action="{{ route('organizer.profile.update-password') }}" class="space-y-4" novalidate>
                @csrf
                @method('PUT')
        ...
        {{-- Logout Section --}}
        <section class="bg-white rounded-2xl p-5 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-205 space-y-4 animate-fade-in-up">
            <h3 class="font-bold text-sm text-gray-800 uppercase tracking-wider border-b border-gray-100 pb-2.5">
                Sesi Akun
            </h3>
