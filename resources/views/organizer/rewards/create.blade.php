<x-app-layout title="Tambah Reward Baru – Jejak Hijau" :user="auth()->user()">
    <div class="px-4 py-6 max-w-3xl mx-auto space-y-6">
        {{-- Breadcrumb & Back --}}
        <div class="flex items-center gap-2 border-b border-gray-150 pb-5">
            <a href="{{ route('organizer.events.rewards.index', $event->id) }}" class="p-2 rounded-xl bg-gray-50 border border-gray-200 hover:bg-gray-100 transition-colors text-gray-500 hover:text-gray-700">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18"></path>
                </svg>
            </a>
            <div>
                <h1 class="text-2xl font-black text-gray-900 tracking-tight">Tambah Reward</h1>
                <p class="text-sm text-gray-500 mt-1">Buat reward baru untuk event <span class="font-semibold text-gray-700">{{ $event->name }}</span>.</p>
            </div>
        </div>
        ...
        <form id="create-reward-form" method="POST" action="{{ route('organizer.events.rewards.store', $event->id) }}" enctype="multipart/form-data" class="bg-white rounded-2xl p-6 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 space-y-6" novalidate>
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: Tumbler Eksklusif GreenRun" required
            ...
            <div class="space-y-1.5">
                ...
                <textarea id="description" name="description" rows="4" placeholder="Deskripsikan reward ini (misal: spesifikasi, cara penukaran)..." required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" id="required_points" name="required_points" value="{{ old('required_points', 100) }}" min="1" required
                ...
                {{-- Stock --}}
                <div class="space-y-1.5">
                    <label for="stock" class="block text-sm font-semibold text-gray-700">Stok Awal <span class="text-red-500">*</span></label>
            ...
            <div class="flex items-center justify-end gap-4 border-t border-gray-100 pt-5">
                <a href="{{ route('organizer.events.rewards.index', $event->id) }}" class="px-5 py-2.5 rounded-xl border border-gray-250 text-sm font-semibold text-gray-700 bg-white hover:bg-gray-50 transition-all">
                    Batal
                </a>
